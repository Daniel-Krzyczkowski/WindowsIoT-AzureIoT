...
static void SendTempAndHumidityConditions(void);
...
static bool isShieldInitialized = false;
static void* sht31;
...
static void DisplayMessage(const char* payload)
{
	// Word display
	clearDisplay();
	setNormalDisplay();
...
static void SetupAzureClient(void)
{
	...
	IoTHubDeviceClient_LL_SetMessageCallback(iothubClientHandle, DisplayMessage, NULL);
}
...
/// </summary>
void SendTempAndHumidityConditions(void)
{

	if (isShieldInitialized == false)
	{
		int i2cFd;
	...
	Log_Debug("Humidity: %.1f\%c\n", newHumidityValue, 0x25);


	char tempBuffer[20];
	int tempDataSize = snprintf(tempBuffer, 20, "%3.2f", newTempValue);

	static char eventBuffer[100] = { 0 };
	static const char* EventMsgTemplate = "%s\: %s";
	int len = snprintf(eventBuffer, sizeof(eventBuffer), EventMsgTemplate, "Temp: ", tempBuffer);

	DisplayMessage(eventBuffer);

	if (tempDataSize > 0)
		SendTelemetry("Temperature", tempBuffer);

	char humidityBuffer[20];
	int humidityDataSize = snprintf(humidityBuffer, 20, "%3.2f", newHumidityValue);
